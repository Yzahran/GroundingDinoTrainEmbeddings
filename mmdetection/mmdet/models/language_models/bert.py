...
class BertModel(BaseModel):
    ...
    def __init__(self,
        ...
        self.train_mode = train_settings["train"]
        self.first_time = False    
        classes = train_settings["classes"]
        replacement_embeddings = train_settings["replacement_embeddings"]
        self.replacement_embeddings = replacement_embeddings
        print("Model is in training mode " if self.train_mode else "Model is in inference mode")
        ...
        if self.train_mode:
            ...
            self.unfreeze_specific_token_embeddings(classes)
        ...
        if self.use_sub_sentence_represent:
            ...
            self.special_tokens = self.tokenizer.convert_tokens_to_ids(
                special_tokens_list)
    def initialize_replacement_embeddings_with_existing(self):
        # Get the embeddings layer from the model
        embeddings = self.language_backbone.body.model.embeddings.word_embeddings

        for new_token, original_token in self.replacement_embeddings.items():
            # Convert tokens to their corresponding IDs in the tokenizer
            new_token_id = self.tokenizer.convert_tokens_to_ids([new_token])[0]
            original_token_id = self.tokenizer.convert_tokens_to_ids([original_token])[0]
            embeddings.weight.data[new_token_id] = embeddings.weight.data[original_token_id].clone()


    def freeze_all_parameters(self):
        for param in self.parameters():
            param.requires_grad = False
    def unfreeze_specific_token_embeddings(self, classes):
        print(f"tokens to unfreeze:{classes}")
        new_token_ids = self.tokenizer.convert_tokens_to_ids(classes)
        for name, param in self.named_parameters():
            if name == "language_backbone.body.model.embeddings.word_embeddings.weight":
                device = param.device  # Get the device of the parameter
                mask = torch.zeros(param.size(0), dtype=torch.bool, device=device)  # Ensure mask is on the same device
