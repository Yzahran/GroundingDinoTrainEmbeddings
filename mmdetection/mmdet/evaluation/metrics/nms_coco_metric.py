...
def apply_custom_nms(image_predictions, iou_threshold=0.5):
    """
    Apply custom NMS to predictions of a single image.
    """
    boxes = image_predictions['bboxes']
    ...
    while len(indices) > 0:
        current = indices[0]
        keep.append(current)
        remaining_boxes = [box for i, box in enumerate(boxes[indices[1:]]) if compute_iou(boxes[current], box) < iou_threshold]
        indices = indices[1:][[i for i, box in enumerate(boxes[indices[1:]]) if compute_iou(boxes[current], box) < iou_threshold]]
    ...
    # Format the results back into the same structure
    nms_predictions = image_predictions.copy()
    nms_predictions['pred_instances'] = {
        'bboxes': nms_boxes,
        'scores': nms_scores,
        'labels': nms_labels
    }
...
def apply_nms_to_predictions(predictions, iou_threshold=0.5):
    """
    Apply custom NMS to predictions of all images.
    """
    nms_predictions = [apply_custom_nms(pred, iou_threshold) for pred in predictions]
    return nms_predictions
...
def convert_to_coco_format(predictions):
    ...
    for prediction in predictions:
        ...
        if 'pred_instances' in prediction and 'bboxes' in prediction['pred_instances']:
            boxes = prediction['pred_instances']['bboxes']  # Convert tensor to numpy
...
class NMSCocoMetric(BaseMetric):
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        for metric in self.metrics:
            ...
            if self.use_mp_eval:
                ...
                nms_dts = apply_nms_to_predictions(preds)
                coco_results_nms = convert_to_coco_format(nms_dts)
                coco_dt_nms  = self._coco_api.loadRes(coco_results_nms)

                coco_eval = COCOeval(self._coco_api, coco_dt_nms, iou_type)
